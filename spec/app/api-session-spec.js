...
describe('测试login-api', () => {
  ...
  it('测试用户名不为空，密码为空', (done) => {
    async.waterfall([
      (user, cb) => request(app).post('/api/session').send({username: '12345972', password: ''}).expect(400, cb),
    ], finish(done))
  });

  it('测试用户名为空，密码不为空', (done) => {
    async.waterfall([
      (user, cb) => request(app).post('/api/session').send({username: '', password: '123456'}).expect(400, cb),
    ], finish(done))
  });

  it('测试密码不提供', (done) => {
    async.waterfall([
      (user, cb) => request(app).post('/api/session').send({username: '12345678'}).expect(400, cb),
    ], finish(done))
  });

  it('测试用户名不提供', (done) => {
    async.waterfall([
      (user, cb) => request(app).post('/api/session').send({ password: '123456'}).expect(400, cb),
    ], finish(done))
  });
